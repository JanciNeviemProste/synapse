...
- **Podklady (PDF/Word/Excel/text):** Peťo nahrá súbory (PDF cez pdf-parse@1.1.1 lazy-loaded, .docx cez mammoth, .xlsx cez exceljs — sheet-by-sheet text s hlavičkami stĺpcov, max 500 riadkov/list, .txt/.md/.csv) → `PetoDoc` (uloží sa len vytiahnutý TEXT, nie binárka — Railway efemérny FS). Pri generovaní sa relevantné časti (keyword retrieval, top 4) priložia ako `knowledge`. Extrakcia: `src/peto/document-text.ts`.
